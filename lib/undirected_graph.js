...
UndirectedGraph = (function() {
  ...
  UndirectedGraph.prototype.canBruteColorize = function(array) {
    var l = array.length, permutations = Math.pow(l, this.size),
        table = [], graph = this.graph;

    for(var x = 0; x < this.size; x++) {
      var t2 = Math.pow(l, x);
      for(var p1 = 0; p1 < permutations;) {
    ...
    for(var i = 0; i < table.length; i++) {
      var positive = true, temp = table[i];
      for(var j = 0; j < this.size; j++) {
        if(_(graph[j].keys()).any(function(k) { return ((j != k) && (temp[k] == temp[j])); })) {
          positive = false;
          break;
        }
      }
